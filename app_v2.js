...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateClocks = () => {
    ...
    try {
      ...
      active.forEach(s => {
        ...
        if (elV) elV.textContent = `${elapsedMin} min`;
      });
    } catch(e) {}
        ...
        if (data && Object.keys(data).length > 0) {
          ...
          if (prevCacheStr !== newCacheStr) {
             ...
             if (activeVisorTab && activeVisorTab.id === 'view-visor-lotes') {
                if (typeof cargarSeccionVisorLotes === 'function') {
  ...
  const abrirModalCargaResultados = async (sampleId) => {
    ...
    currentSampleAnalysesConfig.forEach(a => {
      ...
      if (a.name === 'Humedad') {
        inputHTML = `
          <select class="form-control current-param-input" data-name="${a.name}" style="font-size:0.8rem; height:32px; flex-grow:1; color:#fff; background:rgba(0,0,0,0.2);" required>
        ...
        inputHTML = `
          <input type="number" step="any" class="form-control current-param-input" data-name="${a.name}" placeholder="Ingrese valor" style="font-size:0.8rem; height:32px; flex-grow:1; color:#fff; background:rgba(0,0,0,0.2);" required>
        `;
      }

      divParam.innerHTML = `
        <label style="font-weight:600; color:#fff; display:flex; justify-content:space-between; font-size:0.8rem;">
      ...
      if (a.name === 'Humedad') {
        const createHistSelect = (val, classSuffix) => {
          return `
            <select class="form-control hist-input ${classSuffix}" style="font-size:0.75rem; height:24px; padding:2px 6px; color:#fff; background:rgba(0,0,0,0.2);">
        ...
        histInputsHTML = `
          <div class="form-group" style="margin:0;">
            <label style="font-size:0.65rem;">Ant. -3:</label>
            <input type="number" step="any" class="form-control hist-input hist-val-3" value="${h1}" placeholder="N/A" style="font-size:0.75rem; height:24px; padding:2px 6px; color:#fff; background:rgba(0,0,0,0.2);">
          </div>
          <div class="form-group" style="margin:0;">
            <label style="font-size:0.65rem;">Ant. -2:</label>
            <input type="number" step="any" class="form-control hist-input hist-val-2" value="${h2}" placeholder="N/A" style="font-size:0.75rem; height:24px; padding:2px 6px; color:#fff; background:rgba(0,0,0,0.2);">
          </div>
          <div class="form-group" style="margin:0;">
            <label style="font-size:0.65rem;">Ant. -1:</label>
            <input type="number" step="any" class="form-control hist-input hist-val-1" value="${h3}" placeholder="N/A" style="font-size:0.75rem; height:24px; padding:2px 6px; color:#fff; background:rgba(0,0,0,0.2);">
          </div>
        `;
      }
  ...
  window.abrirModalCargaMasiva = (batchId) => {
    ...
    batchSamples.forEach(s => {
      ...
      tr.innerHTML = `
        ...
        <td><input type="number" step="any" class="form-control batch-param-input" data-param-name="Viscosidad 100°C" style="width:80px; height:24px; font-size:0.75rem; padding:2px 6px; color:#fff; background:rgba(0,0,0,0.2);"></td>
        <td>
          <select class="form-control batch-param-input" data-param-name="Humedad" style="width:85px; height:24px; font-size:0.7rem; padding:2px; color:#fff; background:rgba(0,0,0,0.2);">
            <option value="">Sel...</option>
            <option value="<0.01"><0.01</option>
            <option value=">0.01">>0.01</option>
          </select>
        </td>
        <td><input type="text" class="form-control batch-param-input" data-param-name="Conteo de Partículas (ISO)" placeholder="Ej: 21/19/16" style="width:100px; height:24px; font-size:0.75rem; padding:2px 6px; color:#fff; background:rgba(0,0,0,0.2);"></td>
        <td><input type="text" class="form-control batch-obs-input" placeholder="Ej: Condición normal." style="width:100%; height:24px; font-size:0.75rem; color:#fff; background:rgba(0,0,0,0.2);"></td>
      `;
      tbody.appendChild(tr);
    });
  ...
  const regenerarTablaBatchMuestras = () => {
    ...
    loteMuestrasActuales.forEach((s, idx) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <input type="text" class="form-control item-name" value="${s.name}" style="height:24px; padding:2px 6px; font-size:0.75rem; color:#fff; background:rgba(0,0,0,0.2);">
        </td>
        <td>
          <input type="number" class="form-control item-horometro" value="${s.horometro || ''}" style="height:24px; padding:2px 6px; font-size:0.75rem; width:80px; color:#fff; background:rgba(0,0,0,0.2);">
        </td>
        <td>
          <input type="number" class="form-control item-horometro-cambio" value="${s.hrUltimoCambio || ''}" style="height:24px; padding:2px 6px; font-size:0.75rem; width:80px; color:#fff; background:rgba(0,0,0,0.2);">
        </td>
        <td>
          <input type="text" class="form-control item-ot" value="${s.ot || ''}" style="height:24px; padding:2px 6px; font-size:0.75rem; width:80px; color:#fff; background:rgba(0,0,0,0.2);">
        </td>
        <td>
          <select class="form-control item-cambio-aceite" style="height:24px; padding:2px 6px; font-size:0.75rem; width:85px; color:#fff; background:rgba(0,0,0,0.2);">
        ...
        <td>
          <input type="text" class="form-control item-tipo" value="${s.tipoMuestra || ''}" style="height:24px; padding:2px 6px; font-size:0.75rem; width:85px; color:#fff; background:rgba(0,0,0,0.2);">
        </td>
        <td style="text-align:center;">
  ...
  const cargarSeccionVisorLotes = (isAutoRefresh = false) => {
    ...
    if (uniqueBatchIds.length === 0) {
      ...
      uniqueBatchIds.forEach(bId => {
        ...
        const elapsedMin = (Math.floor((Date.now() - new Date(rep.currentStageStart).getTime()) / 1000) / 60).toFixed(1);
        ...
        flowCard.innerHTML = `
          <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div>
              <h4 style="font-family:var(--font-heading); font-size:1.15rem; color:var(--primary); font-weight:700; display:inline-block;">Lote: ${bId}</h4>
              <span class="badge badge-info" style="margin-left:8px; vertical-align:middle;">${rep.status}</span>
              <p style="font-size:0.75rem; color:var(--text-muted); margin-top:4px;">
                Muestras en lote: <strong>${batchSamples.length}</strong> | Responsable: <strong>${rep.operator}</strong>
              </p>
            </div>
            <div style="text-align:right;">
              <span style="font-size:0.7rem; color:var(--text-muted); display:block;">Etapa actual hace:</span>
              <span style="font-family:monospace; font-size:1.1rem; font-weight:700; color:#b45309;" id="visor-timer-${bId}">${elapsedMin} min</span>
            </div>
          </div>
          <div class="timeline-stages-wrapper">
            <div class="timeline-line"></div>
  ...
  const mostrarDetalleInforme = (sample) => {
    ...
    if (sample.measuredValues && Object.keys(sample.measuredValues).length > 0) {
      resultsHTML = Object.entries(sample.measuredValues).map(([name, val]) => {
        ...
        return `
          <div style="background:rgba(0,0,0,0.2); padding:8px; border-radius:6px; border:1px solid var(--panel-border);">
            <span style="font-size:0.7rem; color:var(--text-muted); display:block;">${name}</span>
            <strong style="font-size:1.1rem; color:#fff; font-family:monospace;">${displayVal}${unit}</strong>
          </div>
        `;
      }).join('');
    } else {
      ...
      resultsHTML = `
        <div style="background:rgba(0,0,0,0.2); padding:8px; border-radius:6px; border:1px solid var(--panel-border);">
          ...
          <strong style="font-size:1.1rem; color:#fff; font-family:monospace;">${viscVal} cSt</strong>
        </div>
        <div style="background:rgba(0,0,0,0.2); padding:8px; border-radius:6px; border:1px solid var(--panel-border);">
          <span style="font-size:0.7rem; color:var(--text-muted); display:block;">Humedad (KF)</span>
          <strong style="font-size:1.1rem; color:#fff; font-family:monospace;">${waterVal} ppm</strong>
        </div>
        <div style="background:rgba(0,0,0,0.2); padding:8px; border-radius:6px; border:1px solid var(--panel-border);">
          <span style="font-size:0.7rem; color:var(--text-muted); display:block;">Conteo de Partículas</span>
          <strong style="font-size:1.1rem; color:#fff; font-family:monospace;">${sample.simulatedValues.iso}</strong>
        </div>
      `;
    }
    ...
    modal.innerHTML = `
      <div class="modal-container" style="max-width: 550px; width: 90%;">
        <div class="modal-header" style="border-bottom:1px solid var(--panel-border); padding-bottom:12px;">
          <h3 style="font-family: var(--font-heading); color: #fff; margin:0;">Informe Analítico: REP-${sample.id}</h3>
        ...
        <div style="padding: 1.5rem; display:flex; flex-direction:column; gap:16px; font-size:0.85rem;">
          ...
          <div class="glass-card" style="padding:1rem; display:grid; grid-template-columns: repeat(2, 1fr); gap:12px; background:rgba(255,255,255,0.02);">
            <div>
              <span style="color:var(--text-muted); display:block; font-size:0.75rem;">Equipo Minero:</span>
              <strong style="color:#fff;">${obtenerNombreEquipo(sample.equipId)}</strong>
            </div>
            <div>
              <span style="color:var(--text-muted); display:block; font-size:0.75rem;">Componente:</span>
              <strong style="color:#fff;">${sample.component}</strong>
            </div>
            <div>
              <span style="color:var(--text-muted); display:block; font-size:0.75rem;">Fecha de Ingreso:</span>
              <span>${new Date(sample.entryTime || Date.now()).toLocaleString('es-ES')}</span>
            </div>
            <div>
          ...
          <div>
            <h4 style="font-family:var(--font-heading); color:#fff; margin-bottom:8px; border-bottom:1px solid var(--panel-border); padding-bottom:4px;">Resultados de Ensayos Físico-Químicos</h4>
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap:10px; text-align:center;">
              ${resultsHTML}
            </div>
          </div>
          
          <div style="background:rgba(16, 185, 129, 0.05); border: 1px solid rgba(16, 185, 129, 0.15); padding: 12px; border-radius:8px;">
            <h5 style="color:#10b981; font-family:var(--font-heading); margin: 0 0 4px 0; font-size:0.85rem; font-weight:700;">Recomendación y Diagnóstico Técnico</h5>
            <p style="margin:0; line-height:1.4; font-size:0.8rem; color:#e2e8f0; font-style:italic;">"${sample.obs}"</p>
          </div>
      ...
      div.innerHTML = `
        ...
        <div style="display: flex; gap: 10px; flex-wrap: wrap; align-items: flex-end;">
          ...
          <div style="flex: 1; min-width: 120px;">
             <button class="btn btn-secondary btn-sm" style="width:100%; height: 38px;" onclick="alert('Lector Excel: En construcción. Inyectará el historial desde marzo a la estadística de este capilar.')"><i data-lucide="upload-cloud"></i> Cargar Historial (Excel)</button>
          </div>
        </div>
      `;
...
function inicializarModuloCapacitaciones() {
  ...
  inputCant.addEventListener('input', () => {
    ...
    for(let i=1; i<=cant; i++) {
      contAsistentes.innerHTML += `
        <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap; background:rgba(255,255,255,0.02); padding:10px; border-radius:8px;">
        ...
        if(cap.nombre.toLowerCase().includes(q) || cap.legajo.toLowerCase().includes(q)) {
          resultadosHTML += `
            <tr>
              ...
              <td>${cap.area}</td>
              <td>${cap.tema}</td>
              <td><span class="badge" style="background:var(--success); color:#fff; font-size:0.7rem; padding: 2px 6px; border-radius:12px;">Recibida</span></td>
              <td>${cap.fecha}</td>
