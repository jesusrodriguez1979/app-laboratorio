...
if (Test-Path $srcPath) {
    ...
    # Escaneamos únicamente la mitad superior (Y de 20 a 500) y con un margen en X (X de 100 a 924)
    # Esto asegura que SÓLO detectemos la rueda y la gota, ignorando el texto "JR-LABS" de abajo
    for ($x = 100; $x -lt 924; $x++) {
        for ($y = 20; $y -lt 500; $y++) {
            ...
            # Si tiene saturación de color (azul del engranaje o dorado de la gota)
    ...
    Write-Host "Tight Bounding Box: MinX=$minX, MaxX=$maxX, MinY=$minY, MaxY=$maxY"
    ...
    if ($maxX -gt $minX -and $maxY -gt $minY) {
        ...
        for ($x = 0; $x -lt $cropW; $x++) {
            for ($y = 0; $y -lt $cropH; $y++) {
                ...
                # Todo lo que no sea el logotipo de color (grisáceo, blanco, o sombras del fondo)
                $isBg = ($diff -lt 35) -or (($pixel.R -gt 200) -and ($pixel.G -gt 200) -and ($pixel.B -gt 200))
        ...
        $dest.Dispose()
        Write-Host "Crop of symbol completed successfully!"
    } else {
        Write-Host "Failed to detect symbol!"
    }
